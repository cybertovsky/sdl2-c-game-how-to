...
struct Sprite stay_on_screen(struct Sprite sprite)
{
    if (sprite.y <= 0)
    {
        sprite.y = 0;
    }
    if (sprite.y >= WINDOW_HEIGHT - sprite.h)
    {
    ...
    if (sprite.x >= WINDOW_WIDTH - sprite.w)
    {
...
int main(int argc, char **argv)
{
    ...
    // struct Sprite gameover = make_sprite(renderer, 30, 30, 100, 30, 0, "gameover.bmp");
    ...
    while (!quit)
    {
            ...
            else if (event.type == SDL_KEYUP)
            {
                ...
                case SDL_SCANCODE_J:
                    for (size_t i = 0; i < 20; i++)
                    {
                        if (fires[i].die == 1)
                        {
                            fires[i].x = (int)(hero.x + hero.w / 2 - fires[i].w / 2);
                            fires[i].y = hero.y;
                            fires[i].die = 0;
                            break;
                        }
        ...
        // SDL_Rect rect_color = {30, 200, 100, 30};
